...
# 불필요한 번역 호출을 방지하기 위한 무시 단어/패턴 목록
IGNORED_PATTERNS = [
    r'^[ㄱ-ㅎㅏ-ㅣ\s]+$',               # ㅋㅋ, ㅎㅎ, ㅠㅠ, ㅇㅇ 등 자음/모음만 있는 경우
    r'^(ok|okay|yes|no|thx|thanks|ty|k|gg|lol|nvm|np|pls|plz|sry)$', # 짧은 영문 리액션 (대소문자 무시)
    r'^(오키|응|어|네|넵|넴|아니|고마워|감사|수고|바이|하이)$',       # 짧은 한글 1~2자 감탄사
]
...
def detect_target_language(text: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    # (한국어가 섞인 한영 혼용 문장도 외국인을 위해 영어로 번역)
